...
import { ChangePasswordResponse, ChangePasswordVars } from './connection';
...
export const ChangePasswordCompletedForm = memo<ChangePasswordCompletedFormProps>(({ className }) => {
  ...
  const [update, { loading }] = useMutation<ChangePasswordResponse, ChangePasswordVars>('/change-password');
  ...
  const { onSubmit, validate } = useMemo<Pick<FormikConfig<ChangePasswordFormValues>, 'onSubmit' | 'validate'>>(() => {
    ...
    return {
      onSubmit: (values, { setErrors, resetForm }) => {
        update({ variables: { password: values.password, newPassword: values.newPassword } })
          .then(() => {
            resetForm();
